...
| D-26 | El fondo FloatingLines (§3.14) usaba el gradiente lavanda `#c1c1ff`→`#e1dfff` de §5.3, a plena luminosidad: las líneas competían con el texto blanco del nodo | **Cerrada por decisión explícita del usuario 2026-07-29:** el campo pasa a dos paletas oscuras y opacas que **siguen el estado de la tarjeta**, azules en `HOY` y rojos en `FUTURO` — `#1d0037 · #00302b · #001227` y `#370000 · #301e00 · #270000`, con fundido de 190ms (el mismo del swap de texto de la ficha). Son **seis hex nuevos, fuera de §5.3**, así que es un desvío consciente de Guardrails §6 y no una derivación del frame: se toma porque §5.3 no tiene ningún oscuro utilizable (su único valor bajo es `--r-line: #464555`) y porque el fondo ganó una función que el spec no le había dado — codificar tiempo, igual que ya hace la tarjeta. Pendiente: reflejarlo en `Spec - Nodo 5` §5.3 del vault |
